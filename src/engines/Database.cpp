...
Vault::Database::configureConnection(const Path &path,
                                     const Parameters &parameters) {
  return HttpConsumer::post(client_, getUrl(Vault::Path{"config/" + path}),
                            parameters);
}
...
std::optional<std::string> Vault::Database::resetConnection(const Path &path) {
  return HttpConsumer::post(client_, getUrl(Path{"reset/" + path}),
                            Parameters{});
}

std::optional<std::string>
Vault::Database::rotateRootCredentials(const Path &path) {
  return HttpConsumer::post(client_, getUrl(Path{"rotate-root/" + path}),
                            Parameters{});
}

std::optional<std::string>
Vault::Database::createRole(const Path &path, const Parameters &parameters) {
...
std::optional<std::string>
Vault::Database::generateCredentials(const Path &path) {
  return HttpConsumer::get(client_, getUrl(Vault::Path{"creds/" + path}));
}

std::optional<std::string>
Vault::Database::createStaticRole(const Path &path,
                                  const Parameters &parameters) {
  return HttpConsumer::post(client_, getUrl(Path{"static-roles/" + path}),
                            parameters);
}
...
std::optional<std::string>
Vault::Database::getStaticCredentials(const Path &path) {
